...
module.exports = function (message, client, prefix, needle) {
  ...
  if (message.content.startsWith(prefix + 'ss')) {
    ...
    if (!args[1]) return message.reply(':x: please specify a domain')

    needle.get(`https://api.c99.nl/createscreenshot?key=${process.env.C99_API_KEY}&url=${args[1]}`, {
    }, (e, r) => {
      ...
      message.channel.send('*Screenshot result sent in your DMs...*');
      const embed = new MessageEmbed();
      embed.setDescription(`Screenshot of: ${args[1]} => ${r.body}`);
