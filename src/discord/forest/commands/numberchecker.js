...
module.exports = function (message, client, prefix, needle) {
  ...
  if (message.content.startsWith(prefix + 'numberchecker')) {

    args = message.content.slice(1).split(' ')

    if (!args[1]) return message.reply(':x: please specify a phone with country code(i.e: +33612345678) (+33 is the country code and it means "0" for France)')

    needle.get(`https://api.c99.nl/phonelookup?key=${process.env.C99_API_KEY}&number=${args[1]}&json`, {
    }, (e, r) => {
      ...
      message.channel.send('*Phone Informations sent in your DMs...*');
