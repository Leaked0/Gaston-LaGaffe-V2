module.exports = function (message, client, prefix, needle) {
  ...
  if (message.content.startsWith(prefix + 'tcp')) {

    args = message.content.slice(1).split(' ')

    if (!args[1]) return message.reply(':x: please specify a host')

    needle.get(`https://check-host.net/check-tcp?host=${args[1]}`, {
      headers: {
      ...
      message.channel.send('*TCP Informations sent in your DMs...*')
      ...
      setTimeout(() => {
        ...
        needle.get(`https://check-host.net/check-result/${String(r.body.request_id)}`, (err, res) => {
          ...
          while(ii < arr.length) {
            ...
            text += `IP adress: \`\`\`${arr[ii].ip||args[1]}\`\`\`\nLocation: ${a[ii][2]}\nFlag: :flag_${a[ii][0]}:\nTime: ${arr[ii].time||"0 ms"}\n\n\n`
            ii++
          ...
          return message.author.send({
            ...
            embed: {
              title: `:white_check_mark: TCP connection check`,
              description: text,
              color: "RANDOM"
            }
